...
__all__ = ['topk_wideresnet']
teacher_options = {'widen_factor':10, 'depth':34, 'dropout_rate':0.3, 'num_classes':100}
student_options = {'widen_factor':4, 'depth':22, 'dropout_rate':0.3, 'num_classes':100}
...
class wide_basic(nn.Module):
    def __init__(self, in_planes, planes, dropout_rate, stride=1, K = 256):
...
class double_threshold(nn.Module):
    ...
    """
    Double threshold function. Given a certain threshold M, it will set to 0 the inputs which are (in absolute value)
    lower than this threshold. The threshold is considere without its sign, i.e. we consider M = abs(M)
    Example:
        >>> a = Variable(torch.rand(4,4)*2-1, requires_grad=True)
        >>> a
        Variable containing:
         0.7000 -0.1018 -0.8101  0.7364
         0.9020 -0.1612  0.8386  0.1223
         0.5029  0.3638 -0.7916 -0.3714
        -0.8285  0.7581 -0.3319  0.4460
        [torch.FloatTensor of size 4x4]
        >>> b = double_threshold(0.5)(a)
        >>> b
        Variable containing:
         0.7000  0.0000 -0.8101  0.7364
         0.9020  0.0000  0.8386  0.0000
         0.5029  0.0000 -0.7916  0.0000
        -0.8285  0.7581  0.0000  0.0000
        [torch.FloatTensor of size 4x4]
        >>> b.sum().backward()
        >>> a.grad
        Variable containing:
    ...
    def forward(self, input):
        return Functional.threshold(input, self.threshold, value=0, inplace=False) \
                            - Functional.threshold(-input, self.threshold, value=0, inplace=False)

    def extra_repr(self):
        inplace_str = "Inverse Threshold with value: {}".format(self.threshold)
        return inplace_str
